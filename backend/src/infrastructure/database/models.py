...
class WorkflowExecutionModel(Base, TimestampMixin):
    ...
    started_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), default=func.now())
    completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    last_error: Mapped[Optional[str]] = mapped_column(Text)
    job_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), index=True, nullable=True)
    candidate_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), index=True, nullable=True)

class WorkflowNodeExecutionModel(Base, TimestampMixin):
    __tablename__ = "workflow_node_executions"
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workflow_execution_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("workflow_executions.id", ondelete="CASCADE"), index=True)
...
class WorkflowEventModel(Base, TimestampMixin):
    ...
    category: Mapped[str] = mapped_column(String(100), index=True)  # System, Agent, Tool
    ...
    metadata_payload: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON)

class LangGraphCheckpointModel(Base, TimestampMixin):
    ...
    checkpoint_payload: Mapped[Dict[str, Any]] = mapped_column(JSONB)
    metadata_payload: Mapped[Dict[str, Any]] = mapped_column(JSONB)
...
class MemoryModel(Base, TimestampMixin, SoftDeleteMixin):
    ...
    out_edges: Mapped[List["MemoryEdgeModel"]] = relationship("MemoryEdgeModel", foreign_keys="[MemoryEdgeModel.source_node_id]", cascade="all, delete-orphan")
    in_edges: Mapped[List["MemoryEdgeModel"]] = relationship("MemoryEdgeModel", foreign_keys="[MemoryEdgeModel.target_node_id]", cascade="all, delete-orphan")
    vectors: Mapped[List["MemoryVectorModel"]] = relationship("MemoryVectorModel", back_populates="memory", cascade="all, delete-orphan")
...
class MemoryVectorModel(Base):
    ...
    memory: Mapped["MemoryModel"] = relationship(back_populates="vectors")
...
class ToolExecutionModel(Base):
    ...
    artifacts: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSONB)
