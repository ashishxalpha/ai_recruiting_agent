...
interface OrganizationWidgetWrapperProps<T> {
  title: string;
  description?: string;
  isLoading: boolean;
  isError: boolean;
...
export function OrganizationWidgetWrapper<T>({ 
  title, 
  description, 
  isLoading, 
  isError, 
...
}: OrganizationWidgetWrapperProps<T>) {
    ...
    return (
      <Card className={`border-dashed ${className}`}>
        ...
        <CardContent className={`flex flex-col items-center justify-center text-center ${contentClassName}`}>
          ...
          <p className="text-xs text-muted-foreground mt-2 max-w-[250px]">{data.metadata.message || "This capability has not yet been integrated into the Organization Studio."}</p>
        </CardContent>
      </Card>
    );
  }

  if (!data.metadata.data_available || !data.data || (Array.isArray(data.data) && data.data.length === 0)) {
    return (
      <Card className={`border-dashed ${className}`}>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2 text-muted-foreground">
        ...
        <CardContent className={`flex flex-col items-center justify-center text-center ${contentClassName}`}>
          ...
          <p className="text-xs text-muted-foreground mt-2 max-w-[250px]">{data.metadata.message || "There are no records in this view."}</p>
        </CardContent>
      </Card>
    );
  }
