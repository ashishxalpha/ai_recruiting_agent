...
interface MemoryWidgetWrapperProps<T> {
  title: string;
  description?: string;
  isLoading: boolean;
  isError: boolean;
  data: MemoryResponse<T> | undefined;
...
export function MemoryWidgetWrapper<T>({ 
  title, 
  description, 
  isLoading, 
  isError, 
  ...
  if (isError) {
    return (
      <Card className={`border-red-500/50 ${className}`}>
        ...
        <CardContent className={`flex items-center justify-center ${contentClassName}`}>
          <p className="text-sm text-red-500/80">Failed to load memory data.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className={`border-dashed ${className}`}>
        ...
        <CardContent className={`flex flex-col items-center justify-center text-center ${contentClassName}`}>
          ...
          <p className="text-xs text-muted-foreground mt-2 max-w-[250px]">{data.reason || "No memory records found."}</p>
        </CardContent>
      </Card>
    );
  }
