...
interface AgentWidgetWrapperProps<T> {
  title: string;
  description?: string;
  isLoading: boolean;
  isError: boolean;
...
export function AgentWidgetWrapper<T>({ 
  title, 
  description, 
  isLoading, 
  isError, 
    ...
    return (
      <Card className={`border-dashed ${className}`}>
        ...
        <CardContent className={`flex flex-col items-center justify-center text-center ${contentClassName}`}>
          ...
          <p className="text-xs text-muted-foreground mt-2 max-w-[250px]">{data.metadata.message || "This capability has not yet been integrated into the Agent Studio."}</p>
        </CardContent>
      </Card>
    );
  }
